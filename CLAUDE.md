...
- Keep the read-only testing/API surface clear: `/api/rag/context`, `/api/rag/collections*`, `/api/rag/documents/[id]/download`, and `/api/rag/harness` are valid because they expose approved context, approved knowledge browsing/downloads, and approved harness visibility. `/api/ollama/*` is valid only as local setup support for the default provider. Do not turn any of these into knowledge-management or harness-management routes; those writes stay MCP-only.
