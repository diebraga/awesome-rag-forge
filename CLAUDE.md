...
- Keep the read-only testing/API surface clear: `/api/rag/context`, `/api/rag/collections*`, `/api/rag/documents/[id]/download`, and `/api/rag/harness` are valid because they expose approved context, approved knowledge browsing/downloads, and approved harness visibility. `/api/chat/providers` is valid only as provider setup metadata/prompt support, and `/api/ollama/*` is valid only as local setup support for the default provider. Do not turn any of these into knowledge-management or harness-management routes; those writes stay MCP-only.
...
- Never let an MCP write path skip `propose_source_insert` → human approval → `PENDING_REVIEW`. Nothing reaches `APPROVED` without an explicit `approve_chunk` call. `reviewTriage` and `READY_FOR_BATCH_APPROVAL` are advisory queue labels only; they must never become auto-approval logic.
