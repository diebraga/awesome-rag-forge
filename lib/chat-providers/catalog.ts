...
export function getProviderSetupPrompt(id: ProviderId): string {
  ...
  return `Set up ${provider.label} for awesome-rag-forge.\n\n1. Open ${provider.setupUrl}.\n2. Create or copy an API key.\n3. Ask me to paste the key into this chat.\n4. Add it to the project's .env file as ${provider.envVar}=<the key>.\n5. Keep the key out of git, logs, screenshots, and browser storage.\n6. If it does not reload automatically, restart the Next.js dev server.\n7. Reload the browser; if ${provider.label} is selected and configured, continue directly to the chat.\n\nUse ${provider.defaultModel} by default. Other lightweight test models: ${provider.models.join(", ")}.`;
}
