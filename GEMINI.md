...
5. **Ask about optional bucket storage after `DATABASE_URL`: buckets are only needed to preserve original PDFs for download; text/OCR ingest works without them. If the user is intentionally exposing the web testing surface online, ask them to set `APP_API_KEY`; if they choose Claude/Codex/Gemini in the local testing UI, ask them for the corresponding provider API key and put it in `.env`, then restart/reload; do not ask for an MCP API key for the default local stdio MCP server. Never fabricate, guess, or auto-generate secrets** (`DATABASE_URL`, `STORAGE_ACCESS_KEY_ID`/`STORAGE_SECRET_ACCESS_KEY`, etc.). These always require the user to obtain them from a real provider and add them to `.env` themselves — point them at [Environment Variables](docs/environment-variables.md) rather than inventing a placeholder that would fail silently later. **Never ask the user to paste a real secret into the chat.** Tell them to run `npm run setup` in their own terminal (masked input, writes straight to `.env`) and verify with `npm run check:env`, which only ever reports connected/failed — see [Security Considerations](docs/security.md#local-secret-onboarding--never-paste-secrets-into-an-ai-assistants-chat).
