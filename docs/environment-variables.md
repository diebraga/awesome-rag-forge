...
- **Never ask the user to paste a real secret into the chat.** For `DATABASE_URL`/storage, tell them to run `npm run setup` in their own terminal. For a hosted provider key (`OPENAI_API_KEY`/`ANTHROPIC_API_KEY`/`GEMINI_API_KEY`), tell them to run `npm run setup:provider -- <id>` instead — see [Security Considerations](security.md#local-secret-onboarding--never-paste-secrets-into-an-ai-assistants-chat). Verify with `npm run check:env`, which only reports connected/failed, never the values.
