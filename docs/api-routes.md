...
`app/collections/page.tsx` (a plain list) and `app/collections/[collectionId]/page.tsx` (paginated documents, expandable to their chunks) use the same `APPROVED`, `EXTERNAL`, `CHAT`-visible read boundary as the chat. Showing collection/document names here is a **deliberate, explicit feature**, not a contradiction of the chat's restriction against narrating its own structure in conversation: that restriction governs natural-language behavior (`lib/rag/chat-context.ts`), not this dedicated browsing view. The detail page also includes a local-only archive action for already-visible approved documents/chunks, guarded by `assertLocalReviewMode()` and confirmed with a reason; it soft-archives rows and is intentionally not an HTTP API route. See [System Architecture](architecture.md#two-audiences-kept-apart-on-purpose) and [Security Considerations](security.md).
...
`POST /api/ollama/install`, `POST /api/ollama/start`, and `POST /api/ollama/pull` are local testing setup helpers for the default Ollama provider: they can install Ollama through a supported local package manager, start the local model server, or pull an allowlisted model when the local-only guard allows it. They never touch Prisma and are not knowledge-management routes. `POST /api/feedback` is the only HTTP route in `app/` that writes to the database, and it can only create a `RagFeedback` row — an end-user reaction to an already-approved answer. There is intentionally no endpoint that creates, edits, approves, rejects, archives, or deletes RAG knowledge or harness rules over HTTP, and there never should be. Collections archive controls are local server actions, not HTTP API routes or OpenAPI surface. The chat app is a retrieval viewer with local model setup helpers plus one narrow feedback-capture exception. Broad knowledge and harness writes go through the MCP server's approval-gated tools — see [MCP Server](mcp-server.md).
