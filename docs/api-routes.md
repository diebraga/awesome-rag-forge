...
1. Calls `buildAssistantContext()` from [`lib/rag/context.ts`](../lib/rag/context.ts) to get the assistant's identity/read-only/harness system prompt, knowledge-base scope stats, and retrieved RAG context.
...
```json
{
  ...
  "capabilities": [
    ...
    "Explain what the knowledge base currently contains."
  ],
  "restrictions": [
